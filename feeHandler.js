...
const {
  ...
  .usage(
    "Usage: $0 --config [path] --gas-price [gwei] --private-key [file] --provider [provider] --query-only --send-to-telegram"
  )
  .demandOption(["config", "privateKey"]).argv;
...
function getProvider() {
  ...
  if (provider === "node") {
    ...
    rpcUrl =
      "https://eth-mainnet.alchemyapi.io/jsonrpc/V1GjKybGLx6rzSu517KSWpSrTSIIXmV7";
  }
...
async function getFeeSharingWallets() {
  const feeSharingWallets = await WrapFeeBurnerInstance.methods
    .getFeeSharingWallets()
    .call();
...
async function setInstances() {
  ...
  NetworkProxyInstance = new web3.eth.Contract(
    KyberNetworkProxyABI,
    KyberNetworkProxyAddress
  );

  const KyberNetworkAddress = await NetworkProxyInstance.methods
  ...
  const FeeBurnerAddress = await NetworkInstance.methods
    .feeBurnerContract()
    .call();
  ...
  WrapFeeBurnerInstance = new web3.eth.Contract(
    WrapFeeBurnerABI,
...
async function validate(reserve) {
  const reserveKNCWallet = await FeeBurnerInstance.methods
    .reserveKNCWallet(reserve)
    .call();
  logger.info(`Reserve KNC Wallet: ${reserveKNCWallet}`);

  const kncWalletBalance = await KNCInstance.methods
    .balanceOf(reserveKNCWallet)
    .call();
  logger.info(
    `Reserve KNC Wallet Balance: ${web3.utils.fromWei(kncWalletBalance)}`
  );

  const kncWalletAllowance = await KNCInstance.methods
    .allowance(reserveKNCWallet, FeeBurnerInstance._address)
    .call();
  logger.info(
    `Reserve KNC Wallet Allowance: ${web3.utils.fromWei(kncWalletAllowance)}`
  );

  const usableKNC = BN.min(
    new BN(kncWalletBalance),
    new BN(kncWalletAllowance)
  );
  logger.info(`Usable KNC in Wallet: ${web3.utils.fromWei(usableKNC)}`);

  const burnFees = await FeeBurnerInstance.methods
    .reserveFeeToBurn(reserve)
    .call();
  logger.info(`Reserve Fees to Burn: ${web3.utils.fromWei(burnFees)}`);

  let sharingFees = new BN(0);
  for (let index in feeSharingWallets) {
    sharingFees = sharingFees.add(
      new BN(
        await FeeBurnerInstance.methods
          .reserveFeeToWallet(reserve, feeSharingWallets[index])
          .call()
      )
    );
  }
  ...
  totalBurnFees = totalBurnFees.add(new BN(burnFees));
  totalShareFees = totalShareFees.add(new BN(sharingFees));

  if (totalFees.gt(new BN(usableKNC))) {
    const text = `VALIDATION ERROR: ${
      configuration.reserve_names[reserve]
    } (${reserve})\n\nUsable KNC (${web3.utils.fromWei(
      usableKNC
    )}) is less than the Total Fees needed (${web3.utils.fromWei(
      totalFees
    )})\n\n`;
    logger.error(text);
    sendTelegram(text);
...
async function doBurnFees(reserve) {
  const fees = new BN(
    await FeeBurnerInstance.methods.reserveFeeToBurn(reserve).call()
  );
  if (fees.gte(new BN(configuration.KNC_MINIMAL_TX_AMOUNT))) {
    try {
      const result = await sendTx(
        FeeBurnerInstance.methods.burnReserveFees(reserve)
      );
...
async function doShareFees(reserve) {
  for (let index in feeSharingWallets) {
    ...
    if (fees.gte(new BN(configuration.KNC_MINIMAL_TX_AMOUNT))) {
      try {
        const result = await sendTx(
          FeeBurnerInstance.methods.sendFeeToWallet(
            feeSharingWallets[index],
            reserve
          )
        );
...
async function main() {
  ...
  if (queryOnly) {
    logger.info(`TOTAL FEES TO BURN: ${web3.utils.fromWei(totalBurnFees)}`);
    logger.info(`TOTAL FEES TO SHARE: ${web3.utils.fromWei(totalShareFees)}`);
    ...
    sendTelegram(
      `KNC Fees Burned: ${web3.utils.fromWei(
        totalBurned
